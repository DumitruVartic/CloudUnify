...
public class FileSystemService
{
    ...
    public async Task<List<UnifiedCloudFile>> ListFilesAsync(string path = "/", string? providerId = null)
    {
        try
        {
            System.Diagnostics.Debug.WriteLine($"Listing files for path: {path}, providerId: {providerId}");
            
            if (providerId != null)
            {
                var files = await _cloudUnify.ListAllFilesAsync(path);
                var filteredFiles = files.Where(f => f.ProviderId == providerId).ToList();
                System.Diagnostics.Debug.WriteLine($"Found {filteredFiles.Count} files for provider {providerId}");
                return filteredFiles;
            }
            
            var allFiles = await _cloudUnify.ListAllFilesAsync(path);
            System.Diagnostics.Debug.WriteLine($"Found {allFiles.Count} files in total");
            foreach (var file in allFiles)
            {
                _fileCache[GetCacheKey(file.Id, file.ProviderId)] = file;
            }
            return allFiles;
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Unexpected error listing files: {ex}");
            return new List<UnifiedCloudFile>();
        }
    }
    ...
    public async Task<UnifiedCloudFile> UploadFileAsync(byte[] content, string fileName, string path, string providerId)
    {
    ...
    private static string GetCacheKey(string fileId, string providerId) => $"{providerId}:{fileId}";
} 